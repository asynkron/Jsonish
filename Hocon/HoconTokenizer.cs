...
public class HoconTokenizer : Tokenizer
{
    ...
    private const string NotInUnquotedTextHead = "\"{}[]=,#`^?!@*&\\";
    private const string NotInUnquotedTextBody = "\"{}[]=,#^?!@*&\\";

    /// Initializes a new instance of the
    /// <see cref="HoconTokenizer" />
    /// class.
    /// </summary>
    /// <param name="text">The string that contains the text to tokenize.</param>
    ...
    public Token PullNext()
    {
        ...
        var rest = _text.Substring(_index);
        throw new FormatException("unknown token: " + rest);
    }
    ...
    public Token PullValue()
    {
        ...
        if (IsUnquotedTextHead()) return PullUnquotedText();
        if (IsArrayStart()) return PullArrayStart();
        if (IsArrayEnd()) return PullArrayEnd();
    ...
    {
        var sb = new StringBuilder();
        while (!EoF && IsUnquotedTextBody()) sb.Append(Take());

        return Token.LiteralValue(sb.ToString());
    }

    private bool IsUnquotedTextHead()
    {
        return !EoF && !IsWhitespace() && !IsStartOfComment() && !NotInUnquotedTextHead.Contains(Peek());
    }

    private bool IsUnquotedTextBody()
    {
        return !EoF && !IsWhitespace() && !IsStartOfComment() && !NotInUnquotedTextBody.Contains(Peek());
    }
